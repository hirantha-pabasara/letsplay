...
if (isset($_SESSION["user"])) {
    ...
    if ($address_num == 1) {
        ...
        Database::iud("UPDATE `user_has_address` SET `Aline1`='" . $line1 . "', `Aline2` = '" . $line2 . "',`city_id` = '" . $city . "'
        WHERE `user_email`= '" . $_SESSION["user"]["email"] . "'");

    } else {
        Database::iud("INSERT INTO `user_has_address`
    (`Aline1`,`Aline2`,`user_email`,`city_id`) VALUES 
    ('" . $line1 . "','" . $line2 . "','" . $_SESSION["user"]["email"] . "','" . $city . "')");
    }
